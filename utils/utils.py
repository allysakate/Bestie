...
def refine_label_generation(seg_map, center_map, offset_map, label, gt_seg_map, args):
    ...
    for b in range(B):
        ...
        for _cls in _valid_cls:
            ...
            n_contours, contours, stats, centroids = cv2.connectedComponentsWithStats(
                __fg_mask, connectivity=8
            )
            ...
            for k in range(1, n_contours):
                ...
                __ins_seg = get_instance_segmentation(
                    contour_mask[None, ...],
...
def refine_label_generation_with_point(
    seg_map, gt_point_cls, offset_map, label, gt_seg_map, args
):
    """
    ...
    for b in range(B):
        ...
        for cls in _valid_cls:
            ...
            _gt_point = [
                (gt_y, gt_x)
                for gt_y, gt_x in _gt_point_cls[cls]
...
def get_instance_segmentation(
    fg, ctr_hmp, offsets, threshold=0.1, nms_kernel=3, top_k=None, ignore=True, beta=5
):
    """
    # This implementation is from https://github.com/bowenc0221/panoptic-deeplab.
    ...
    if beta > 0:  # center clustring
        try:
            ctr_cluster = cluster_peaks(
                offsets[0].cpu().numpy(), fg[0].cpu().numpy(), beta=beta
            )
            ctr_cluster = np.int32(
                [[cy, cx] for cy, cx in ctr_cluster if ctr_hmp[0, 0, cy, cx] > 0.05]
            )
            ...
            if ctr_cluster.size(0) > 0:
                ...
                if ctr.size(0) == 0:
                    ...
                    for c_cluster in ctr_cluster:
                        c_min_dist = torch.norm(
                            ctr.float() - c_cluster.float(), dim=-1
                        ).min()

                        if c_min_dist > 100:
                            new_ctr = torch.cat(
                                [new_ctr, c_cluster.unsqueeze(0)], dim=0
                            )
                            ctr_hmp[
                                0, 0, c_cluster[0], c_cluster[1]
                            ] = 1.0  # mark as new peak
        except:  # noqa E722
            new_ctr = ctr
    else:
        new_ctr = ctr
...
def cluster_peaks(offset_map, fg, thresh=2.5, beta=5):
    ...
    nLabels, labels, stats, centroids = cv2.connectedComponentsWithStats(
        weak_dp_region.astype(np.uint8), connectivity=4
    )
...
def get_ins_map(out, cls_label, target_size, device, args):
    ...
    if args.val_flip:
        seg_prob = (seg_prob[0] + seg_prob[1].flip(-1)) / 2.0  # [C+1, H, W]
        center_map = (center_map[0] + center_map[1].flip(-1)) / 2.0  # [C, H, W]
    else:
        seg_prob = seg_prob[0]
