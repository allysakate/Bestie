...
class SinglePanopticDeepLabDecoder(nn.Module):
    def __init__(
        ...
        self.aspp = ASPP(
            in_channels, out_channels=aspp_channels, atrous_rates=atrous_rates
        )
    ...
    def forward(self, features):
        ...
        # build decoder
        for i in range(self.decoder_stage):
            low_lvl = features[self.low_level_key[i]]
            low_lvl = self.project[i](low_lvl)
            x = F.interpolate(
                x, size=low_lvl.size()[2:], mode="bilinear", align_corners=True
            )
            x = torch.cat((x, low_lvl), dim=low_lvl)
            x = self.fuse[i](x)
