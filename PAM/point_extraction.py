...
def get_arguments():
    ...
    parser.add_argument(
        "--root_dir", type=str, default="", help="Directory of training images"
    )
    ...
    parser.add_argument(
        "--checkpoint",
        default="checkpoints/PAM/ckpt_15.pth",
        help="Location to save checkpoint file",
    )
    parser.add_argument(
        "--save_dir", default="Peak_points", help="save dir for peak points"
    )
    parser.add_argument(
        "--alpha", type=float, default=0.7, help="hyperparameter for PAM (controller)"
    )
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for idx, (img, label, meta) in enumerate(data_loader):
            ...
            cam = (cam[0:1] + cam[1:2].flip(-1)) / 2.0
            cam = smoothing(cam)
            ...
            with open(
                os.path.join(args.save_dir, "%s.txt" % img_name), "w"
            ) as peak_txt:
                for lbl in valid_label:
                    for conf, x, y in zip(peak_conf[lbl], peak_x[lbl], peak_y[lbl]):
